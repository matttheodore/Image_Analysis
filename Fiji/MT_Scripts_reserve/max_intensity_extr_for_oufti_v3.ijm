...
for (i = 0; i < fileList.length; i++) {
  ...
  for (j = 1; j <= n; j++) {
	...
	// naming all open max files
	imageTitles = newArray(nImages()-1);
	for (k = 0; k < nImages()-1; k++) {
    imageTitles[k] = getTitle();
		}
			for (k = 0; k < nImages()-1; k++) {
    		if (startsWith(imageTitles[k], "MAX")) {
        	selectWindow(imageTitles[k]);
        	saveAs("Tiff", myDirsingle_file + imageTitles[k]);
        	close();
    }
    close();
}
