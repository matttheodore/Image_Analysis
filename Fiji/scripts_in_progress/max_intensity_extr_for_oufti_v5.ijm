...
// Create an array of the titles of all open images that start with 'MAX'
maxImageTitles = newArray(0);
for (k = 1; k <= nImages(); k++) {
    selectImage(k); // Select image by ID, not by title
    title = getTitle();
    if (startsWith(title, "MAX")) {
        maxImageTitles = Array.concat(maxImageTitles, title);
    }
}

// Loop over the 'MAX' images, save them, and then close them
for (k = 0; k < maxImageTitles.length; k++) {
    // Use selectWindow to select image by title
    selectWindow(maxImageTitles[k]);
    saveAs("Tiff", myDirsingle_file + maxImageTitles[k]);
    close();
}

      // Save the image sequence and name it according to its channel
      saveAs("Tiff", myDirsingle_file + "Max_Intensity_Sequence_" + title);
      close();
  }
}
